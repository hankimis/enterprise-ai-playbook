...
"AI 티"는 막연한 인상이 아니라 *측정 가능*하다. IOV Labs \#link("https://labs.iovstudio.kr/ko/papers/ai-design-tells")[*The Tells*] 연구는 이를 7개 패밀리·*21개 텔(tell)*로 분해하고 *Tell Score*(0~100, 낮을수록 좋음, 가중합)로 정량화한다.
...
#table(
  ...
  align: left,
  stroke: 0.4pt + luma(210),
  table.header([*패밀리*], [*대표 텔 (가중치)*]),
  [A 색], [인디고/바이올렛 기본 팔레트(9), 블루→퍼플 히어로 그라데이션(7)],
  [B 타이포], [Inter/Roboto/시스템 기본(9), 타입 스케일 규율 없음(5)],
  [C 레이아웃], [히어로+3카드 템플릿(8), 센터 정렬 일변도(5), 단일 라운드값(4), 이모지 아이콘(3)],
  [D 스페이싱], [균일 카드 패딩(5), 균일 섹션 리듬(3)],
  [E 표면], [제네릭 확산 그림자(5), 헤어라인/포커스 상태 결여(6), 글래스모피즘 남용(4)],
  [F 모션], [인터랙션 마이크로상태 결여(7), 전부 페이드(4)],
  [G 카피], [모호한 열망형 헤드라인(6), 플레이스홀더 텍스트(5), 제네릭 CTA(4)],
)

*핵심 결과*: 같은 페이지에서 콘텐츠·구조는 그대로 두고 *텔 요소만 교체*했더니 Tell Score가 *76(F등급) → 0(A등급)*으로 떨어졌다(교란 통제 리팩터). 사람이 디자인한 실제 사이트(Stripe·Linear·Notion 등) *202곳은 중앙값 0*(93% A등급)인데 AI 기본 페이지는 35~59에 머문다.
...
== 3.2 UI/UX에서 AI 슬롭 피하기 `[3.1.1 The Tells로 보강]`
...
수치는 기업·보도 발표이며, *"목표"와 "실측"을 구분*해 읽어야 한다.
...
#block(fill: luma(245), inset: 8pt, radius: 3pt, width: 100%)[정직: 위 수치는 자체·벤더 발표다. 특히 "30%"는 목표치, "20/12.5/11%"는 실측 보고지만 독립 감사는 아니다. *자사 베이스라인 대비 직접 측정*으로 재검증하라.]
...
*제조 (한국 강세)*
- 우선 유스케이스: 결함·불량 비전검사, 예지보전, 수율 분석, 생산계획 최적화(스마트 팩토리)
- 함정: 데이터·OT 통합, 현장 안전, 파일럿→라인 확장의 격차

*금융*
- 우선: 실시간 사기탐지(오탐↓), 문서·심사 보조, 고객응대, 컴플라이언스. 회수 빠른 편(보고상 ~90일)
...
#block(fill: luma(245), inset: 8pt, radius: 3pt, width: 100%)[*공통 교훈(정직)*: Klarna는 "AI 단독" 전략을 철회하고 *하이브리드로 회귀*했다. 복잡·감정적 문의는 인간 판단이 필요했고, 하이브리드가 총 산출이 더 많았다. 또 레거시 시스템 *통합이 확장의 1차 장벽*(딜로이트 60%). 업종 불문, 통제·HITL·측정이 ROI의 전제다.]
...
*디자인·업종·사례*: The Tells (IOV Labs, labs.iovstudio.kr/papers/ai-design-tells, hankimis/ai-design-tells, 21텔·Tell Score·202사이트 검증), 한국 도입 사례(삼성·GS칼텍스·HD현대미포·TYM·현대차·ABACUS, 국내 보도), 업종별 유스케이스(Menlo·McKinsey·Glean·CIO, JPMorgan·Walmart·Klarna 보도).
